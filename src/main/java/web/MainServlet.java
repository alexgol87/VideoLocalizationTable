...
public class MainServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        if ((thread.getState() == Thread.State.NEW || thread.getState() == Thread.State.TERMINATED) && req.getParameter("runUpdate").equals("yes")) {
            ...
            if (req.getParameter("creativeType").equals("banner")) thread = new Thread(taskBanner);
            if (req.getParameter("creativeType").equals("community")) thread = new Thread(taskCommunity);
